...
- `POST /v1/admin/anti-cheat/eac/report` — service/admin token with `CUSTOM:ADMIN:NAMESPACE:{namespace}:ANTICHEAT:EAC [CREATE]` permission, requires `sessionId` unless session validation is disabled. Verifies `userId` is in the session (no leader requirement).
...
- To call admin endpoint: `CUSTOM:ADMIN:NAMESPACE:{namespace}:ANTICHEAT:EAC [CREATE]`.
